...
def move_to_target(target_position):
    ...
    Kp_angle, Ki_angle, Kd_angle = 0.1, 0.01, 0.05
    Kp_dist, Ki_dist, Kd_dist = 0.1, 0.01, 0.05
    ...
    while True:
        ...
        desired_angle = math.degrees(math.atan2(target_y - current_y, target_x - current_x))
        ...
        if abs(angle_error) > angle_threshold:
            ...
            if angle_error > 0:
                publish_controller_data((0, 0, max(0.15, min(angle_correction, 1)), 0, 0))  # Tilt right
            else:
                publish_controller_data((0, 0, max(-1, min(angle_correction, -0.15)), 0, 0))  # Tilt left
