...
class MyController(Controller):
    ...
    def on_R3_x_at_rest(self):
        self.R3_value[0] = 0
        self.wheels,self.angle, self.power = self.calc_wheels_speed(self.R3_value[0],self.R3_value[1],self.L3_value)
    ...
    def on_R2_press(self, value):
        self.R2_value = self.map_stick_value(value)
        if self.new_data_callback is not None:
            self.new_data_callback()
    ...
    def map_stick_value(raw_value):
        ...
        # Apply dead zone
        if abs(mapped_value) < 0.06:
            return 0
        return mapped_value
    
    @staticmethod
    # function to convert x and y values to angle
    def calc_wheels_speed(x, y, rotation, power=None,angle=None): 
        angle = math.atan2(y, x) if angle is None else angle
        power = math.hypot(x, y) if power is None else power
        ...
        return (leftFront, rightFront, leftRear, rightRear), angle, power
