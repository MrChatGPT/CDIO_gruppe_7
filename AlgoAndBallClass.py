...
class Ball:
    ...
    def __repr__(self):
        return f"Ball(x={self.x}, y={self.y}, obstacle={self.obstacle})"
...
#         data = json.load(file)
#     # Convert the nested list of lists back to a list of Ball objects
#     coordinates = [Ball(*center) for center in data]
#     return coordinates
...
# Balls = LoadBalls("whiteballs.json")
# OrangeBall = LoadBalls("orangeballs.json")[0]  # Only one orange ball, therefore take only the first element
...
# # Print each ball values (x, y and bvalue) for debugging
# for ball in Balls:
#     print(ball)
...
def BallValue(ball, TrackCorners, TrackEdges, ObstaclePoints, CenterPoint):
    ...
    for point, value in TrackCorners.items():
        if Distance(ball.position, point) < 100:  # Arbitrary distance to be considered "close"
            return value
    ...
    if Distance(ball.position, TopPoint) < 100 and Distance(ball.position, CenterPoint) < 100 and Distance(ball.position, LeftPoint) < 100:
        return 13
    
    # Check if the ball is in the top-right inner corner
    if Distance(ball.position, TopPoint) < 100 and Distance(ball.position, CenterPoint) < 100 and Distance(ball.position, RightPoint) < 100:
        return 14
    
    # Check if the ball is in the bottom-left inner corner
    if Distance(ball.position, LeftPoint) < 100 and Distance(ball.position, CenterPoint) < 100 and Distance(ball.position, BottomPoint) < 100:
        return 15
    
    # Check if the ball is in the bottom-right inner corner
    if Distance(ball.position, BottomPoint) < 100 and Distance(ball.position, CenterPoint) < 100 and Distance(ball.position, RightPoint) < 100:
        return 16
...
# Plot the balls and their obstacles
def PlotBallsAndValues(Balls, OrangeBall, RobotXY):
    ...
    for ball in Balls:
        plt.plot(ball.x, ball.y, 'go')  # Green circles for white balls
        plt.text(ball.x + 10, ball.y + 10, str(ball.obstacle), fontsize=12, color='green')

    # Plot the orange ball
    plt.plot(OrangeBall.x, OrangeBall.y, 'o', color='orange', label='Orange Ball')  # Orange circle for orange ball
    plt.text(OrangeBall.x + 10, OrangeBall.y + 10, str(OrangeBall.obstacle), fontsize=12, color='orange')  # Orange text for value
    ...
    plt.title('Ball Positions and Their Obstacles')
...
def DistanceWithPenalty(robot, ball, obstacles):
    BaseDistance = Distance(robot, ball.position)
...
def SortByDistance(RobotXY, Balls, OrangeBall, ObstacleXY):
    # Sort the balls based on distance to the robot with penalty for crossing the obstacle
    SortedList = sorted(Balls, key=lambda ball: DistanceWithPenalty(RobotXY, ball, ObstacleXY))
    
    # Add the orange ball at the end
    SortedList.append(OrangeBall)
...
SortedBalls = SortByDistance(RobotXY, Balls, OrangeBall, ObstacleXY)

# Update each Ball object with its calculated Bvalue
for ball in SortedBalls:
    ball.obstacle = BallValue(ball, TrackCorners, TrackEdges, ObstaclePoints, CenterPoint)


# Create a final list with updated Ball objects
FinalList = SortedBalls


# # Print the final sorted list with coordinates and Bvalues
print("Final sorted list :", FinalList)

# Plot the balls and their obstacles for debugging
PlotBallsAndValues(FinalList, OrangeBall, RobotXY)


# Function to move the robot to the target position
def move_to_target(target_ball):
    print(f"Moving to target position: {target_ball.position} with Bvalue: {target_ball.obstacle}")
    
# # Call the function to move to the target
FirstBall = FinalList[0]
move_to_target(FirstBall)
